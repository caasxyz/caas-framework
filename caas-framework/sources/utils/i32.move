module dex_contract::i32 {
    ...
    #[deprecated]
    public fun i32_res(_v: I32): I32_RES {
    ...
    #[deprecated]
    public fun or(_num1: I32, _num2: I32): I32 {
        abort error::unavailable(EDEPRECATED)
    }

    #[deprecated]
    public fun and(_num1: I32, _num2: I32): I32 {
        abort error::unavailable(EDEPRECATED)
    }

    public fun round_to_spacing(num: I32, spacing: u32, up: bool): I32 {
        // ((num / spacing) + (up ? 1 : -1)) * spacing
        let quotient = div(num, from_u32(spacing));
        let remain = mod(num, from_u32(spacing));
        let round = if (eq(remain, zero())) { false }
        else { true };
        let increment = if (up) {
            from_u32(1)
        } else {
        ...
        let intermiate = add(quotient, increment);
        mul(intermiate, from_u32(spacing))
    }
